	...
	beforeEach(() => {
		try {
			...
			({
				...
				/* @ts-expect-error - Argument of type '{ _dependencies: { dependency: jest.Mock<any, any, any> | null; }; }' is not assignable to parameter of type 'useButtonAnimationsInput'. */
				useButtonAnimations({ _dependencies: { dependency: mockDependency } }),
			));
		} catch (thrown) {
