 ...
 */
export type ArrowRightEventHandler<T = Element> = EventHandler<ArrowLeftEvent<T>> &
	KeyboardEventHandler<T>;

/**
 * Checks that an `unknown` value is an {@link ArrowRightEventHandler} function.
 *
 * Requirements:
 ...
 *
 * @returns The determination that `value` is or is not an {@link ArrowRightEventHandler} function.
 */
export const isArrowRightEventHandler = <T = Element>(
	value: unknown,
	event?: Record<string, unknown>,
): value is ArrowRightEventHandler<T> =>
	/**
	 * value
	 */
	event
